...
public class UnweightedGraph<V> implements Graph<V> {
    ...
    /** method for DFS search with stack */
    ...
    private void dfs(int v, int[] parent, List<Integer> searchOrder,
        ...
        while (!stack.isEmpty()) {
            ...
            if (!isVisited[element]) {
                searchOrder.add(element);
                isVisited[element] = true;
            }

            List<Edge> elementNeighbors = neighbors.get(element);
            for (int i = 0; i < elementNeighbors.size(); i++) {
                Edge n = elementNeighbors.get(i);
                Integer neighbor = n.v;
